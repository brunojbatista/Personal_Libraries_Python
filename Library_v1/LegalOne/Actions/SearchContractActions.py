...
class SearchContractActions(LegalOneActions):
    def __init__(self, driver: DriverInterface = None) -> None:
        ...
        self.xpath = XPath()
        self.xpath.screen("search_contract")
    ...
    def navigate_search_simple(self, ): # ok
        self.navegate_search_page()
        button_filter = self.get_element(self.xpath.get("filtro_simples"))
        if not self.has_class(button_filter, "active"):
            self.click_element_with_loading(
                self.xpath.get("filtro_simples"),
                self.xpath.get("filtro_carregamento"),
            )
        return self;

    def navigate_search_advanced(self, ): # ok
        self.navegate_search_page()
        button_filter = self.get_element(self.xpath.get("filtro_avancado"))
        if not self.has_class(button_filter, "active"):
            self.click_element_with_loading(
                self.xpath.get("filtro_avancado"),
                self.xpath.get("filtro_carregamento"),
            )
        return self;

    def search_simple_by_situation(self, *situations): # ok
        DropdownMultipleSelectionField(self.get_driver(), self.xpath.get("lookup_situacao_simples")).set_value(*situations)
        self.click_element(self.xpath.get("botao_pesquisar"))
        return self;

    def get_contracts(self, ): # ok
        contracts = []
        rows_el = self.get_elements(self.xpath.get("tabela_resultados"), time=10)
